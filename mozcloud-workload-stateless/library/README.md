# mozcloud-workload-stateless-lib

This chart provides templates to build stateless MozCloud Workloads. Stateless workloads define `Deployments`/Argo Rollouts, which are short-lived and don't carry any state (e.g. persistent volumes).
...
type: application
dependencies:
  - name: mozcloud-workload-stateless-lib
    version: ~0.4.7
...
> [!IMPORTANT]
> MozCloud library charts usually provide templates for a matching application chart, e.g. [mozcloud-workload-stateless-lib](https://github.com/mozilla/helm-charts/tree/main/mozcloud-workload-stateless/library) is meant to include supporting templates for [mozcloud-workload-stateless](https://github.com/mozilla/helm-charts/tree/main/mozcloud-workload-stateless/application). Because of that, templates in library charts often expect the same contexts as their application counterparts.
